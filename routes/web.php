...
Route::group(['prefix'=>'admin','middleware'=>['CheckLogedOut','role:Admin|Writer|Editer|Publisher']], function(){
    ...
    /*--------------------Quản lý bình luận--------------------*/
    Route::get('comment', [CommentController::class, 'getComment'])->middleware('permission:quan ly binh luan');
    Route::get('comment/{id}/detail', [CommentController::class, 'detailComment'])->middleware('permission:quan ly binh luan');
    Route::post('comment/delete', [CommentController::class, 'delComment'])->middleware('permission:quan ly binh luan');
...
// Tìm kiếm sản phẩm
Route::post('/tim-kiem', 'App\Http\Controllers\home\HomeController@search');
